...
const { InferenceClient } = require('@huggingface/inference'); // Импортируем InferenceClient
const fetch = require('node-fetch'); // node-fetch все еще нужен для InferenceClient

exports.handler = async function(event, context) {
  // Получаем Hugging Face API токен из переменных окружения Netlify
  const HUGGING_FACE_API_TOKEN = process.env.HUGGING_FACE_API_TOKEN;
  // const API_URL = '...'; // Больше не нужен прямой URL, используем клиент

  // Инициализация клиента Inference
  ...
  try {
    // Вызов Chat Completion через InferenceClient и провайдера Nebius
    const chatCompletion = await client.chatCompletion({
        ...
        parameters: {
            ...
            top_p: 0.9,              
            do_sample: true,         
            // return_full_text: false // Этот параметр не используется в chatCompletion
        },
    ...
    // Обработка ошибок, которые могут возникнуть от InferenceClient
